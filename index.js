import { info, timeout, parseFile, generateRandomAmount, privateToAddress,  log, getTrueAmount} from './tools/other.js';
import { fromWei, getETHAmount, getEstimateGas, getGasPrice, getPriorityGasPrice, numberToHex, sendEVMTX, toWei} from './tools/web3.js';
...
const withdrawToChain = async(chain, toAddress, privateKey) => {
    ...
    const random2 = await getTrueAmount(info.rpcLinea, address, 'Swap');
    const rpc = info['rpc' + chain];
    const eip1559 = ['Ethereum', 'Arbitrum', 'Optimism', 'Polygon', 'Linea', 'Sepolia', 'Manta']
    ...
    try {
        await getETHAmount(rpc, address).then(async(amountETH) => {
            await getGasPrice(rpc).then(async(gasPrice) => {
                ...
                const gasLimit = chain == 'Arbitrum' || chain == 'zkSync' || chain == 'Linea' || chain == 'Sepolia'
...
(async() => {
    ...
    for (let i = 0; i < wallet.length; i++) {
        ...
        if (index == 0) {
            ...
            await withdrawToChain('Manta', walletCEX[i], wallet[i]);
        }
        // else if (index == 10) {
